...
type Props = {
   ...
   *     PlayerRatings: (props) => <PlayerRatings {...props} motm={frontmatter.motm} />
   *   }} />
   *
...
export default function PlayerRatings({ ratings, motm }: Props) {
  return (
      ...
      {ratings.map((r) => (
        <li key={r.playerId} className={r.playerId === motm ? "player-ratings__item is-motm" : "player-ratings__item"}>
          <span className="player-ratings__name">
            {r.name}
            {r.playerId === motm && <StampBadge kind="motm" size="small" />}
          </span>
